...
export const validateUsername = (username: string): void => {
    ...
    if (!/^[a-zA-Z0-9_.-]+$/.test(username)) {
        throw new Error("Username can only contain alphanumeric characters, underscore, hyphen, period, and no whitespace");
    }

    // Is not reserved
    const lowercaseUsername = username.toLowerCase();
    if (lowercaseUsername === "admin" || lowercaseUsername === "administrator") {
...
export const validatePassword = (password: string): void => {
    ...
    if (password.length < minLength) {
        throw new Error(`Password (${password.length}) must be at least ${minLength} character(s)`);
    }
    // Maximum length
    if (password.length > maxLength) {
        throw new Error(`Password (${password.length}) must be at most ${maxLength} characters`);
    }
}
