...
import { ScopesType, jwtAccessSetup, jwtRefreshSetup } from "@/plugins/auth";
...
const token = new Elysia({ prefix: '/token' })
    ...
    .use(jwtRefreshSetup)
    .use(bearer())
    .post('/refresh', async ({ jwtAccess, jwtRefresh, set, bearer, body }) => {
...
const validateScopes = async (
    ...
    if (scopes['account:modify'] || scopes['account:delete']) {
        ...
        if (!refreshToken.iat || refreshToken.iat < Math.floor(Date.now() / 1000) - 5 * 60) {
            throw new Error("Account modification requires recent refresh token");
        }
    }
};
