...
const login = new Elysia()
    ...
    .use(
        rateLimit({
            // TODO: Implement login throttle (double exponential with 1 second base)
            max: 1,
            duration: 60,
    ...
    .post(
        "/login",
        async ({ body, jwtRefresh, logger }) => {
            const username = body.username;
            const password = body.password;

            // biome-ignore lint/suspicious/noImplicitAnyLet: harder to read otherwise
            let user;
            try {
                ...
                logger.debug(
                    `Error occurred while trying to find user: ${errorMsg}`,
                );
                throw error;
            }
            ...
            try {
                ...
                throw new InternalServerError(
                    `Error occurred while trying to verify password: ${errorMsg}`,
                );
            }
    ...
    if (usernameOrEmail.includes("@")) {
        ...
        if (!normalizedEmail) {
            throw new CredentialsError(
                "Invalid email. If you mean to enter a username, please remove the @ symbol",
            );
        }
