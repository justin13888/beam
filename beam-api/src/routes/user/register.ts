import { normalizeUsername, validateEmail, validatePassword, validateUsername } from "@/lib/validators/user";
...
const register = new Elysia()
    ...
    .post('/register', async ({body, jwtRefresh }) => {
    ...
    if (!normalizedEmail) {
        throw new ParseError("Invalid email: Failed to normalize email");
    }
    const normalizedUsername = normalizeUsername(username);

    // Validate email is not already in use
    const emailExists = await db.select().from(users)
        .where(eq(users.email, normalizedEmail)).execute();
    ...
    const existingUser = await db.select().from(users)
        .where(eq(users.username, normalizedUsername)).execute();
